...
const ROMAJI_MAP = {
  ...
  'ヤ': 'ya', 'ユ': 'yu', 'ヨ': 'yo',
  'ラ': 'ra', 'リ': 'リ', 'ル': 'ru', 'レ': 're', 'ロ': 'ro',
  'ワ': 'wa', 'ヲ': 'wo', 'ン': 'n',
  // Katakana Dakuon (Fixed: 'び' -> 'ビ' and other duplicates corrected)
...
const VOICE_QUIZ_WORDS_KATAKANA = [
  ...
  { word: 'カレー', meaning: '카레', variants: ['카레', 'カレー'] },
  { word: 'ラーメン', meaning: '라면', variants: ['라면', '라면'] },
  ...
  { word: 'マイク', meaning: 'マイク', variants: ['マイク', '마イク'] },
  ...
  { word: 'スキー', meaning: '스키', variants: ['스키', '스키'] },
  ...
  { word: 'スポーツ', meaning: '스포츠', variants: ['스포츠', '스포츠'] }
];
            ...
            <div className="space-y-6 pb-12">
                <div className="space-y-3">
                  ...
                  <div className="bg-white border border-slate-100 rounded-3xl p-4 shadow-sm space-y-4">
                    ...
                    <button onClick={() => startSession('GAME_RANDOM_ALL')} className="w-full py-4 bg-slate-900 text-white rounded-2xl flex items-center justify-center gap-2 text-xs font-black shadow-lg hover:bg-slate-800 hover:-translate-y-1 active:scale-95 transition-all"><Dices className="w-4 h-4 text-slate-400" /> 랜덤 챌リンジ (60s)</button>
                  </div>
                </div>
